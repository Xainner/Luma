...
export async function initDb(): Promise<void> {
  await pool.query(`
    CREATE TABLE IF NOT EXISTS app_config (
      ...
      model_thinking JSONB NOT NULL DEFAULT '{}'::jsonb
    );
    ...
    ALTER TABLE app_config ADD COLUMN IF NOT EXISTS thinking_effort TEXT NOT NULL DEFAULT 'medium';
    ALTER TABLE app_config ADD COLUMN IF NOT EXISTS model_thinking JSONB NOT NULL DEFAULT '{}'::jsonb;
    ...
    ALTER TABLE user_config ADD COLUMN IF NOT EXISTS thinking_effort TEXT NOT NULL DEFAULT 'medium';
    ALTER TABLE user_config ADD COLUMN IF NOT EXISTS model_thinking JSONB NOT NULL DEFAULT '{}'::jsonb;
...
export async function loadUserConfig(userId: string): Promise<AppConfig> {
  const { rows } = await pool.query(
    'SELECT base_url, api_key, model, temperature, max_tokens, profile_id, language, thinking_effort, model_thinking FROM user_config WHERE user_id = $1',
    [userId],
  )
  return rows.length ? { ...defaultConfig, ...rowToConfig(rows[0]) } : { ...defaultConfig }
}
